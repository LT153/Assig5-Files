...
/*
    ...
    public void RecWriteTree(ObjectOutputStream OS, BinaryNode<T> node) {
        ...
        {
            try {		// Recursively output left subtree
    ...
    public void buildInorder(String filename) {
        try {
            ...
            root = recBuildTree((T[]) array, 0, array.length - 1);
            ois.close();
        } catch (IOException e) {
    ...
    private BinaryNode<T> recBuildTree(T[] array, int start, int end) {
        if (start > end) {
            return null;
        }
        int mid = start + (end - start) / 2;
        BinaryNode<T> left = recBuildTree(array, start, mid-1);
        BinaryNode<T> parent = new BinaryNode(array[mid]);
        parent.setLeftChild(left);
        BinaryNode<T> right = recBuildTree(array, mid+1, end);
        ...
        /*BinaryNode<T> node = null;
         ...
         node = new BinaryNode(array[start]);
         } else {
         int nodePosition = (int) Math.floor(size / 2.0);
         node = new BinaryNode(array[nodePosition]);
         node.setLeftChild(recBuildTree(array, start, nodePosition - 1));
         node.setRightChild(recBuildTree(array, nodePosition + 1, end));
         }
